...
function getDays() {
    ...
    for (let i = 1; i < firstDay.getUTCDay(); i++) {
        if (i < firstDay.getUTCDay()) {
            // Pega a posição do primeiro dia da semana e se o index for menor, adiciona o valor 0
            days.push(0);
        }
    }

    for (let i = 0; i <= lastDay.getUTCDate(); i++) {
        // Pega o ultimo dia do mês para saber quantos dias teremos
        days.push(i); // adiciona o valor do index ao dia
    }

    while (days.length % 7 != 0) {
        // Se a quantidade de dias divido por 7 não tiver um resto 0
...
function createTable() {
    ...
    for (let i = 0; i < days.length; i++) {
        unavailableDay =
            ...
            ((days[i] <= date.getDate() &&
                ...
                year < date.getFullYear());
        if (days[i] != 0) {
            // se o valor dia for diferente de 0
            if (diaSemana == 0 || diaSemana == 6) {
                // Se o dia da semana for um final de semana
        ...
        if (diaSemana != 6) {
            // Se o dia da semana for diferente de 6
    ...
    let presetDayWeek = document.querySelector(
        `div[value='${date.getDate() + 1}']`
    );
    if (presetDayWeek.classList.contains("weekend")) {
        presetDayWeek = document.querySelector(
            `div[value='${date.getDate() + 2}']`
        );
        if (presetDayWeek.classList.contains("weekend")) {
            presetDayWeek = document.querySelector(
                `div[value='${date.getDate() + 3}']`
            );
...
function nextMonth() {
    ...
    if (month < 11) {
        // se o mês for menor que 11 (ultimo mês)
    ...
    if (selectedDay != undefined) {
        document
            .querySelector(`div[value= "${selectedDay.getAttribute("value")}"]`)
            .classList.add("selectedDay");
    }
}

function previousMonth() {
    // Mesma coisa da função de cima mas para diminuir os valores
    clearTable();
    if (month > 0) {
        month--;
    } else {
...
function actualDay() {
    ...
    document.getElementById("select-month").style.backgroundPosition =
        "55px center";
}
...
let appointments = [
    // array com as marcações de presença
    {
        day: 12,
        month: 3,
        year: 2021,
...
function appointmentSubmit() {
    // Trata os dados depois do submit no modal
    ...
    if (name.value == "" || image.value == "") {
        // Se o nome ou a url não estiverem preenchidos
...
function dayClick(e) {
    // Captura o click no dia
    selectedDay = e; // Coloca o dia na variavel
    for (let i = 0; i < table.children.length; i++) {
        // Let para percorrer todos os elementos da tabela
        if (table.children[i].classList.contains("selectedDay")) {
            // Se alguma div tiver a classe
...
function weekMode() {
    ...
    if (checkWeekMode == true) {
        ...
        for (let i = 0; i <= days.length; i++) {
            // laço de repetição com os todos os dias da tabela;
...
function setWeeks() {
    // Configura em qual semana está cada dia
    let week = 0; // inicia a semana em 0 (primeira semana do mês)

    for (let i = 0; i <= days.length; i++) {
        // Percorre todos os dias do array
        if (i >= 7 * week && i < 7 * (week + 1)) {
            // Mesma logica da função acima
...
function loadAppointments() {
    appointments.forEach((e) => {
        ...
        if (e.month == month && e.year == year) {
            if (
                day.classList.contains("unavailableDay") ||
                day.classList.contains("weekend")
            ) {
                if (!day.children[0]) {
